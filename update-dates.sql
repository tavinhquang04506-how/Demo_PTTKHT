...
-- 1. Xóa dữ liệu vé cũ trước (do foreign key)
...
-- 3. Cập nhật ngày khởi chiếu cho phim đang chiếu
...
-- 4. Cập nhật ngày khởi chiếu cho phim sắp chiếu
...
-- 5. Tạo lại suất chiếu từ 07/04/2026
...
-- Kiểm tra kết quả
SELECT 'Phim' as loai, ten_phim, ngay_khoi_chieu, trang_thai FROM phim ORDER BY ngay_khoi_chieu;
SELECT 'Suất chiếu' as loai, COUNT(*) as tong, MIN(ngay_chieu) as tu_ngay, MAX(ngay_chieu) as den_ngay FROM suat_chieu;
